...
merged_tsv="$(gh api "/repos/${UPSTREAM}/pulls" --paginate \
    -f state=closed -f sort=updated -f direction=desc -f per_page=100 \
  | jq -r '[.[] | select(.merged_at != null and .base.ref == "master")]
            | sort_by(.merged_at)[]
            | [.number,.title,.merged_at,.merge_commit_sha,.html_url] | @tsv'
)"
...
while IFS=$'\t' read -r n title merged_at msha url; do
  [ -n "$n" ] || continue
  ...
  if git rev-parse --verify "refs/remotes/origin/${branch}" >/dev/null 2>&1; then
    ...
    git fetch --no-tags upstream "$msha" --quiet || die "не могу получить merge_commit ${msha}"
