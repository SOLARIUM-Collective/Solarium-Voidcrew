...
/datum/unit_test/vestige_biology_route/birth/Run()
	...
	for(var/prey_index in 1 to 2)
		...
		TEST_ASSERT(UNLINT(prey.base_ranged_item_interaction(user, birth.egg, list())) & ITEM_INTERACT_BLOCKING, "The actual ranged egg handler must accept the hunt input.")
...
/datum/unit_test/vestige_biology_route/faces/Run()
	...
	for(var/exchange in 1 to 2)
		...
		UNLINT(prey.base_ranged_item_interaction(user, proboscis, list()))
		if(exchange == 1)
...
/datum/unit_test/vestige_biology_route/proc/dose(obj/item/reagent_containers/syringe/vestige_precision/syringe, obj/item/reagent_containers/cup/bottle/bottle, obj/structure/vestige_tissue_culture/culture, volume)
	...
	while(volume > 0)
		...
		if(syringe.amount_per_transfer_from_this != portion)
			return FALSE
		if(!(UNLINT(bottle.base_item_interaction(user, syringe, list(RIGHT_CLICK = "1"))) & ITEM_INTERACT_SUCCESS))
			return FALSE
		if(!(UNLINT(culture.base_item_interaction(user, syringe, list())) & ITEM_INTERACT_SUCCESS))
			return FALSE
		volume -= portion
	return TRUE
...
/datum/unit_test/vestige_biology_route/field_study/Run()
	...
	TEST_ASSERT(UNLINT(culture.base_item_interaction(user, probe, list())) & ITEM_INTERACT_SUCCESS, "The actual probe must assay the first measured dose.")
	...
	TEST_ASSERT(UNLINT(culture.base_item_interaction(user, probe, list())) & ITEM_INTERACT_SUCCESS, "The actual probe must assay the buffer response.")
	...
	TEST_ASSERT(UNLINT(culture.base_item_interaction(user, probe, list())) & ITEM_INTERACT_SUCCESS, "The actual probe must assay the prepared culture.")
	TEST_ASSERT_EQUAL(culture.viability, 70, "Three necessary assays must leave a viable preparation.")
	TEST_ASSERT(UNLINT(culture.base_item_interaction(user, probe, list(RIGHT_CLICK = "1"))) & ITEM_INTERACT_SUCCESS, "The real right-click probe input must harvest the balanced live culture.")
...
/datum/unit_test/vestige_biology_route/acquisition/Run()
	...
	for(var/index in 1 to 3)
		...
		TEST_ASSERT(UNLINT(barrier_spot.base_item_interaction(user, panels[index], list())) & ITEM_INTERACT_SUCCESS, "The real panel input must build each of the first three enclosure sides.")
	...
	TEST_ASSERT(UNLINT(final_barrier.base_item_interaction(user, panels[4], list())) & ITEM_INTERACT_SUCCESS, "The actual fourth panel must close the last escape.")
	...
	TEST_ASSERT(UNLINT(specimen.base_ranged_item_interaction(user, lens, list())) & ITEM_INTERACT_SUCCESS, "The actual lens must certify the live, herded, enclosed specimen.")
...
/datum/unit_test/vestige_biology_route/graft/Run()
	...
	TEST_ASSERT(UNLINT(patient.base_item_interaction(user, dossier, list())) & ITEM_INTERACT_BLOCKING, "The diagnostic input must reject the sick, unopened patient.")
	...
	TEST_ASSERT(UNLINT(patient.base_item_interaction(user, dossier, list())) & ITEM_INTERACT_SUCCESS, "The actual dossier must certify the recovered, closed, living specimen.")
