...
/datum/unit_test/vestige_exterior_route/long_dark/Run()
	...
	TEST_ASSERT(UNLINT(trial.echo.base_item_interaction(user, trial.shard, list())) & ITEM_INTERACT_SUCCESS, "Touching the echo with the held shard must finish its real recovery channel.")
	TEST_ASSERT(trial.recovered, "The recovery channel must put the echo in the shard.")
	user.forceMove(center)
	TEST_ASSERT(UNLINT(trial.home_beacon.base_item_interaction(user, trial.shard, list())) & ITEM_INTERACT_SUCCESS, "Returning the recovered shard must deliver it.")
...
/datum/unit_test/vestige_exterior_route/other_side/Run()
	...
	TEST_ASSERT(UNLINT(pane.base_item_interaction(user, trial.card, list())) & ITEM_INTERACT_SUCCESS, "The actual three-second inside press must leave a reflection.")
	...
	TEST_ASSERT(UNLINT(pane.base_item_interaction(user, trial.card, list())) & ITEM_INTERACT_SUCCESS, "The actual exterior press must collect the reply.")
	TEST_ASSERT_EQUAL(trial.phase, 2, "The second completed press must require a return inside.")
	user.forceMove(center)
	TEST_ASSERT(UNLINT(pane.base_item_interaction(user, trial.card, list())) & ITEM_INTERACT_SUCCESS, "The actual final inside press must deliver the reply.")
...
/datum/unit_test/vestige_exterior_route/little_moon/Run()
	...
	for(var/pull in 1 to 4)
		...
		TEST_ASSERT(UNLINT(cargo.base_ranged_item_interaction(user, trial.tether, list())) & ITEM_INTERACT_SUCCESS, "Each real cardinal cable pull must succeed.")
	...
	user.forceMove(home)
	TEST_ASSERT(UNLINT(trial.cradle.base_item_interaction(user, trial.keepsake, list())) & ITEM_INTERACT_SUCCESS, "The held keepsake must deliver to its actual launch cradle.")
...
/datum/unit_test/vestige_morph/perfect_copy_route/Run()
	...
	for(var/role in 1 to 2)
		...
		TEST_ASSERT(UNLINT(original.base_item_interaction(user, trial.skin, list())) & ITEM_INTERACT_SUCCESS, "The held skin must copy the requested original through actual item dispatch.")
