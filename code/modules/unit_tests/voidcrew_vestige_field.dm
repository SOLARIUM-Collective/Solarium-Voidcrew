...
/datum/unit_test/vestige_field_route/stillness/Run()
	...
	for(var/exchange in 1 to 3)
		...
		TEST_ASSERT(UNLINT(trial.actor.base_item_interaction(user, trial.incense, list())) & ITEM_INTERACT_SUCCESS, "Touching the instructor with the held incense must counter.")
		if(exchange < 3)
...
/datum/unit_test/vestige_field_route/snuffed_flame/Run()
	...
	for(var/index in 1 to 9)
		...
		TEST_ASSERT(UNLINT(lamp.base_item_interaction(user, trial.censer, list())) & ITEM_INTERACT_SUCCESS, "The held censer must toggle through actual item dispatch.")
...
/datum/unit_test/vestige_field_route/unseen_hand/Run()
	...
	TEST_ASSERT(UNLINT(trial.actor.base_item_interaction(user, trial.seal, list())) & ITEM_INTERACT_SUCCESS, "The actual held seal must finish its rear channel during the corner pause.")
...
/datum/unit_test/vestige_field_route/the_watched/Run()
	...
	TEST_ASSERT(UNLINT(northwest_lamp.base_item_interaction(user, trial.eye, list())) & ITEM_INTERACT_SUCCESS, "The held eye must extinguish the actual lamp.")
	...
	TEST_ASSERT(UNLINT(trial.actor.base_ranged_item_interaction(user, trial.eye, list())) & ITEM_INTERACT_SUCCESS, "The first rear haunting must use the real ranged channel.")
	...
	TEST_ASSERT(UNLINT(southeast_lamp.base_item_interaction(user, trial.eye, list())) & ITEM_INTERACT_SUCCESS, "The second real lamp must be extinguished through the eye.")
	...
	TEST_ASSERT(UNLINT(trial.actor.base_ranged_item_interaction(user, trial.eye, list())) & ITEM_INTERACT_SUCCESS, "The second rear haunting must use a new real repair channel.")
	...
	TEST_ASSERT(UNLINT(focus.base_item_interaction(user, trial.eye, list())) & ITEM_INTERACT_SUCCESS, "The real eye input must finish the encounter after both searches.")
...
/datum/unit_test/vestige_field_route/long_night/Run()
	...
	TEST_ASSERT(UNLINT(first_refuge.base_item_interaction(user, trial.glass, list())) & ITEM_INTERACT_SUCCESS, "The actual glass input must charge at refuge one.")
	...
	for(var/transfer in 2 to 4)
		...
		TEST_ASSERT(UNLINT(refuge.base_item_interaction(user, trial.glass, list())) & ITEM_INTERACT_SUCCESS, "The held glass must transfer through the actual next-refuge input.")
		if(transfer < 4)
...
/datum/unit_test/vestige_field_ranged_haunting/Run()
	...
	var/result = UNLINT(trial.actor.base_ranged_item_interaction(user, trial.eye, list()))
...
/datum/unit_test/vestige_field_tether_dispatch/Run()
	...
	blocker.density = TRUE
	var/blocked_result = UNLINT(trial.cargo.base_ranged_item_interaction(user, trial.tether, list()))
	var/blocked_drift = trial.cargo.drift_x
	qdel(blocker)
	var/result = UNLINT(trial.cargo.base_ranged_item_interaction(user, trial.tether, list()))
