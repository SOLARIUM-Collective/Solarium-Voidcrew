...
/datum/unit_test/vestige_morph_route/snatched_meal/Run()
	...
	TEST_ASSERT(UNLINT(lure.base_ranged_item_interaction(user, trial.maw, list())) & ITEM_INTERACT_SUCCESS, "The held maw must spit its real scent onto the nearest legal lure tile.")
	...
	TEST_ASSERT(UNLINT(trial.pantry.base_item_interaction(user, trial.maw, list())) & ITEM_INTERACT_SUCCESS, "The real theft channel must remove the guarded course.")
...
/datum/unit_test/vestige_morph_route/understudy/Run()
	...
	TEST_ASSERT(UNLINT(trial.actor.base_item_interaction(user, trial.skin, list())) & ITEM_INTERACT_SUCCESS, "The first skin interaction must begin the custodian's real demonstration.")
	...
	TEST_ASSERT(UNLINT(trial.actor.base_item_interaction(user, trial.skin, list())) & ITEM_INTERACT_SUCCESS, "The second skin interaction must finish its real identity-study channel.")
	...
	for(var/index in 1 to length(shipment))
		...
		TEST_ASSERT(UNLINT(destination.base_item_interaction(user, parcel, list())) & ITEM_INTERACT_SUCCESS, "The real receiving-tray interaction must accept the held marked parcel.")
	...
	TEST_ASSERT(UNLINT(trial.input.base_item_interaction(user, trial.skin, list())) & ITEM_INTERACT_SUCCESS, "The held skin must operate the actual release.")
