...
/datum/unit_test/vestige_rites_route/offering/Run()
	...
	TEST_ASSERT(UNLINT(center.base_item_interaction(user, chalk, list())) & ITEM_INTERACT_SUCCESS, "The actual eight-second chalk channel must scribe a rune.")
	...
	user.execute_mode()
	for(var/obj/item/flashlight/flare/candle/candle as anything in candles)
		TEST_ASSERT(UNLINT(candle.base_item_interaction(user, lighter, list())) & ITEM_INTERACT_SUCCESS, "The actual lit lighter must ignite each grounded candle.")
...
/datum/unit_test/vestige_rites_route/last_breath/Run()
	...
	for(var/index in 1 to 3)
		...
		corpse.death()
		TEST_ASSERT(UNLINT(corpse.base_item_interaction(user, lantern, list())) & ITEM_INTERACT_SUCCESS, "The actual three-second lantern channel must collect each distinct substantial corpse's breath.")
...
/datum/unit_test/vestige_rites_route/sitters_rounds/Run()
	...
	UNLINT(patient.base_item_interaction(user, cloth, list()))
	...
	for(var/medicine_type in list(/obj/item/stack/medical/bruise_pack/vestige_sitter, /obj/item/stack/medical/ointment/vestige_sitter))
		...
		for(var/application in 1 to 10)
			...
			TEST_ASSERT(UNLINT(patient.base_item_interaction(user, dressing, list())) & ITEM_INTERACT_SUCCESS, "The real dressing interaction must start ordinary treatment on the selected injured limb.")
...
/datum/unit_test/vestige_rites_route/rite_of_toll/Run()
	...
	for(var/step in 1 to 14)
		...
		TEST_ASSERT(UNLINT(casket.base_item_interaction(user, tool, list())) & ITEM_INTERACT_SUCCESS, "The real two-second tool channel must work the lock.")
...
/datum/unit_test/vestige_rites_route/true_vigil/Run()
	...
	UNLINT(corpse.base_item_interaction(user, vigil.candle, list()))
	...
	for(var/obj/structure/vestige_mourning_rift/rift as anything in rifts)
		...
		TEST_ASSERT(UNLINT(rift.base_item_interaction(user, vigil.candle, list())) & ITEM_INTERACT_SUCCESS, "Each actual three-second candle channel must close its rift while the others keep processing.")
...
/datum/unit_test/vestige_rites_route/true_vigil_group_transit/Run()
	...
	TEST_ASSERT(UNLINT(first.base_item_interaction(user, vigil.candle, list())) & ITEM_INTERACT_SUCCESS, "The carried candle must still close a rift after complete group transit.")
...
/datum/unit_test/vestige_rites_route/true_vigil_split_z/Run()
	...
	var/candle_result = UNLINT(first.base_item_interaction(user, vigil.candle, list()))
	...
	candle_result = UNLINT(first.base_item_interaction(user, vigil.candle, list()))
	...
	corpse.forceMove(center)
	TEST_ASSERT(UNLINT(first.base_item_interaction(user, vigil.candle, list())) & ITEM_INTERACT_SUCCESS, "Returning the remains must let the same physical candle close the original rift.")
...
/datum/unit_test/vestige_rites_route/singed_hand/Run()
	...
	for(var/index in 1 to 3)
		...
		UNLINT(miscast.base_ranged_item_interaction(user, geode, list()))
		TEST_ASSERT(QDELETED(miscast), "Actual ranged geode input must capture each spent manifestation.")
		if(index == 2 || index == 3)
			UNLINT(lesson.focus.base_item_interaction(user, geode, list()))
			if(index == 2)
...
/datum/unit_test/vestige_rites_route/steady_tongue/Run()
	...
	for(var/obj/structure/vestige_miscast/miscast as anything in miscasts)
		for(var/push in 1 to 6)
			...
			TEST_ASSERT(UNLINT(miscast.base_item_interaction(user, primer, list())) & ITEM_INTERACT_SUCCESS, "The actual ready spoken word must repel the manifestation: [word_state], destination [expected], blocked [expected?.is_blocked_turf(exclude_mobs = TRUE)].")
			if(!QDELETED(miscast))
...
/datum/unit_test/vestige_rites_route/swallowed_word/Run()
	...
	for(var/number in puzzle_solution)
		...
		TEST_ASSERT(UNLINT(glyph.base_item_interaction(user, phial, list())) & ITEM_INTERACT_SUCCESS, "Every computed slide must succeed through actual adjacent phial dispatch.")
...
/datum/unit_test/vestige_rites_route/rite_of_rust/Run()
	...
	for(var/anointing in 1 to 2)
		...
		TEST_ASSERT(UNLINT(wall.base_item_interaction(user, chrism, list())) & ITEM_INTERACT_SUCCESS, "Each actual two-second anointing must rust the ordinary wall.")
...
/datum/unit_test/vestige_rites_route/rite_of_transcription/Run()
	...
	TEST_ASSERT(UNLINT(door.base_item_interaction(user, quill, list())) & ITEM_INTERACT_SUCCESS, "The real five-second first reading must register the access-restricted threshold.")
	...
	TEST_ASSERT(UNLINT(door.base_item_interaction(user, screwdriver, list())) & ITEM_INTERACT_SUCCESS, "The actual screwdriver must open the engineering panel.")
	...
	TEST_ASSERT(UNLINT(door.base_item_interaction(user, crowbar, list())) & ITEM_INTERACT_SUCCESS, "The actual crowbar tool dispatch must pry the disabled door.")
	...
	TEST_ASSERT(UNLINT(door.base_item_interaction(user, crowbar, list())) & ITEM_INTERACT_SUCCESS, "The same actual crowbar must close the disabled airlock.")
	for(var/tick in 1 to 30)
		if(door.density && !door.operating)
			break
		sleep(1)
	...
	TEST_ASSERT(UNLINT(door.base_item_interaction(user, quill, list())) & ITEM_INTERACT_SUCCESS, "The real opposite-side five-second reading must transcribe the restored refusal.")
